...
const showProgress = (downloadedBytes, totalBytes) => {
  ...
  gauge.show(
    `Downloading Chromium - ${toMegabytes(downloadedBytes)} / ${toMegabytes(
      totalBytes
    )}`,
    per
  );
...
const downloadNow = async (option, browserFetcher) => {
  ...
  await browserFetcher
    .download(option.revision, (downloadedBytes, totalBytes) => {
      downloading = true;
  ...
  if (localRevisions && localRevisions.length) {
    ...
    localRevisions = localRevisions.filter(
      (revision) => revision !== option.revision
    );
...
const downloadHandler = async (option) => {
  ...
  if (!res) {
    output(
      `ERROR: Failed to download Chromium after retry ${option.retryTimes} times.`,
      true
    );
  }
};
...
const revisionHandler = (option) => {
  ...
  if (typeof option.launchable === "boolean") {
    ...
    const launchable = revisionInfo.launchable
      ? EC.green("true")
      : EC.red("false");
...
const PCR = async (option = {}) => {
  ...
  if (stats && fs.existsSync(stats.executablePath)) {
    ...
    if (
      !option.revision ||
      (option.revision && option.revision === stats.revision)
    ) {
      return stats;
    }
  }
